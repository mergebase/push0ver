...
public class TagExtractor {
    ...
    public static Map<String, Tag> getTag(String gitDir, String projectDir, boolean releaseExists, MyLogger log, String[] badTag) {
        ...
        try {
            ...
            in = process.getInputStream();
            isr = new InputStreamReader(in, "UTF-8");
            Map<String, Tag> tags = extractTag(projectDir, isr, releaseExists, log, badTag, currentBranch);
    ...
    private static String parentDir(String s) {
        if ("".equals(s)) {
            return null;
        }
        int x = s.lastIndexOf('/');
    ...
    public static void validateAndCleanTags(Map<String, Tag> tags) {
        if (tags == null) {
            return;
        }
        Set<String> validChildren = new HashSet<>();
        for (Map.Entry<String, Tag> entry : tags.entrySet()) {
            String dir = entry.getKey();
            Tag tag = entry.getValue();
            String parent = parentDir(dir);
            if (parent != null) {
                Tag parentTag = tags.get(parent);
                if (parentTag != null) {
                    if (tag.versionEquals(parentTag)) {
                        validChildren.add(dir);
                    } else {
                        String pt = parent + "=" + parentTag.getVersion();
                        String ct = dir + "=" + tag.getVersion();
                        throw new IllegalArgumentException("Incompatible parent/child tag combo. " + pt + " but " + ct);
                    }
                }
            }
        }

        for (String s : validChildren) {
            tags.remove(s);
        }
    }

    static Map<String, Tag> extractTagTestLogicFull(Reader r) {
